...
const minimizedCountries = require("./data/minimizedCountries.json");

let lux = minimizedCountries.find((country) => country.metadata.name === "Afghanistan")
// lux = lux.minimizedPoints.map((point) => [point.lng, point.lat]);
const geojsonlux = {
  type: "Feature",
  properties: {
  ...
  geometry: {
    type: "Polygon",
    coordinates: [lux.minimizedPoints.map((point) => [point.lng, point.lat])],
  }
};

require("fs").writeFileSync("afghanistan.geojson", JSON.stringify(geojsonlux, null, 2));
require("fs").writeFileSync("afghanistan.json", JSON.stringify(lux, null, 2));
// exit(0)
